...
import 'package:sam_rider_app/driver_src/util/map_util.dart';
import 'package:sam_rider_app/driver_src/util/utils.dart';

import 'chat.dart';

class Driver_MyHomePage extends StatefulWidget {
  Driver_MyHomePage({Key key, this.selectedData}) : super(key: key);
...
class _MyHomePageState extends State<Driver_MyHomePage> {
  _MyHomePageState();
  ...
  void registerEarningListener() async {
    subscription = FirebaseDatabase.instance
        ...
        .onValue
        .listen((event)  {
          if(event.snapshot.value["weeklyEarning"] != null)
            {
              setEarning(event);
            }
    });
  ...
  void setEarning(Event event) {
    setState(() {
      earning = event.snapshot.value["weeklyEarning"].toString();
      mPrice = earning;
    });
  }
  ...
  void getWeeklyEarning() {
    dataBloc.getWeeklyEarning().then((value) {
      setState(() {
        if (value != null) {
          print("earn: " + value);
          earning = value;
  ...
  void notifyClient(LatLng place) {
    if (isStarted && isInited) {
      if (fromLocation != null && clientToken != "") {
        if (!notified) {
          if (calculateDistance(place.latitude, place.longitude) <= 2) {
            dataBloc.notifyClient(clientToken);
            notified = true;
          }
        }
      }
  ...
  double calculateDistance(lat2, lon2) {
    ...
    var lon1 = fromLocation.longitude;
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        isInited || selectedData != null
                            ...
                            : SizedBox(width: 60),
                        PriceWidget(
                          price: (mPrice == null || mPrice == "null") ? "0" : mPrice ,
                          onPressed: () {},
                        ),
                        ProfileWidget(
                          onPressed: () =>
