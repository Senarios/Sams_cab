...
class _YourCurrentTripPageState extends State<YourCurrentTripPage> {
  ...
  Uint8List destinationDriverPinIcon;
  Timer timer;
  var isDone = false;
  var isInited = false;
  ...
  void initState() {
    ...
    setCustomTargetPin();
    setCustomDriverPin();
    print("Here: " + data["driver_id"] + data["profile"]);
                  ...
                  child: SizedBox(
                      ...
                      child: AppStyle.requestCard(context, data, onMessage: () {
                       onChat(data["driver_id"], data["profile"]);
                        ...
                        dataBloc.completeRide(data["data_id"], onSuccess);
                      }))))
        ],
      ),
    );
  ...
  void onSuccess()
  {
    ...
    Navigator.pushReplacementNamed(context, '/your_trips_list');
  }
  ...
  void _addMarker(String mkId, LatLng place) async {
    ...
    if (mkId == "from_address") {
      ...
      Marker marker = Marker(
        ...
        //LatLng(place.lat, place.lng),
        infoWindow: InfoWindow(title: mkId),
        icon: BitmapDescriptor.fromBytes(destinationDriverPinIcon),
        anchor: const Offset(0.1, 1),
      );
      _markers.add(marker);
    }
